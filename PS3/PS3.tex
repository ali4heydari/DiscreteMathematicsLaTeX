...
\begin{align*}
 ...
 B &: HumanoidWolf(C) \\
  ...
  \begin{cases}
    ...
    \neg Lier(A) \wedge Fact, & \mbox{otherwise}.
 \\
 &\Rightarrow \quad HumaoidWolf(A)
  \end{cases}
\end{align*}
\end{answer}
\end{shaded}
...
\begin{answer}
\begin{align}
 Fact \label{Fact 1} &: \quad \exists x \in S : \quad (\forall y \in S : \quad IsHumanoidWolf(y) \quad \Leftrightarrow \quad x = y) \quad \Rightarrow \neg Lier(x) \\
 Fact \label{Fact 2} &: \quad \exists y , z \in S : (y , z \neq x ) \wedge Lier(y) \wedge Lier(z) \\
 B &: \quad IsHumanoidWolf(C)
\end{align}
\begin{align*}
&\Rightarrow \quad
\begin{cases}
  HumanoidWolf(C) \wedge \ref{Fact 2} \; \text{\textreferencemark}, & \mbox{if } \neg Lier(B) \\
  \neg HumaoidWolf(C) \wedge \ref{Fact 1} \; \text{\textreferencemark}, & \mbox{otherwise}.
\end{cases}
